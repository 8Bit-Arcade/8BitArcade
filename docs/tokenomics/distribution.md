# Token Distribution

## How 500M 8BIT Tokens Are Allocated

Complete breakdown of where every token goes and when you can access them.

## Distribution Overview

```
Total Supply: 500,000,000 8BIT (100%)

├─ 200,000,000 (40%) - Token Sale
├─ 200,000,000 (40%) - Rewards & Ecosystem
│  ├─ 150,000,000 (30%) - Daily Rewards + Tournaments (5 YEAR DISTRIBUTION)
│  └─ 50,000,000 (10%) - Staking Rewards
└─ 100,000,000 (20%) - Initial Distribution
   ├─ 60,000,000 (12%) - DEX Liquidity (locked 3+ years)
   ├─ 20,000,000 (4%) - Tournament Prize Pools
   ├─ 15,000,000 (3%) - Marketing & Partnerships
   └─ 5,000,000 (1%) - Team (vested 2-3 years)
```

## Detailed Breakdown

### 🎯 Token Sale - 200,000,000 (40%)

**Purpose:** Public presale to raise development capital

| Detail | Information |
|--------|-------------|
| **Amount** | 200,000,000 8BIT |
| **Price** | $0.0005 per token |
| **Max Raise** | $100,000 USD |
| **Minimum Buy** | $100 USD (200,000 8BIT) |
| **Vesting** | None - tokens available immediately |
| **Network** | Arbitrum One |

**Where Funds Go:**
- 75% → DEX Liquidity ($75,000)
- 15% → Marketing & Listings ($15,000)
- 10% → Development & Operations ($10,000)

**Why 40% is Reasonable:**
- Ensures wide token distribution
- Prevents single-holder dominance
- Raises sufficient capital for professional launch
- Standard for successful token launches

---

### 🏆 Rewards Pool - 150,000,000 (30%)

**Purpose:** Daily free rewards + tournament prizes

| Detail | Information |
|--------|-------------|
| **Amount** | 150,000,000 8BIT |
| **Distribution Period** | 5 years (60 months) |
| **Monthly Emissions** | 2,500,000 8BIT |
| **Daily Emissions** | ~82,000 8BIT |
| **Lock/Vest** | Time-locked smart contract |

**Monthly Breakdown:**
- **Daily Rewards:** 1,500,000 8BIT (50K per day)
- **Standard Tournaments:** 180,000 8BIT
- **High Roller Tournaments:** 820,000 8BIT

**Why 5-Year Distribution:**
- Front-loaded rewards benefit early players
- Most projects don't last 20+ years
- Maximizes earning potential NOW
- Creates urgency and excitement
- Still sustainable with deflationary burns

**After Year 5:**
- No new emissions from this pool
- Circulating supply stabilizes
- Deflationary pressure increases
- Token becomes increasingly scarce

---

### 💎 Staking Pool - 50,000,000 (10%)

**Purpose:** Long-term holder incentives and passive income

| Detail | Information |
|--------|-------------|
| **Amount** | 50,000,000 8BIT |
| **Distribution Period** | 5 years |
| **Launch Timeline** | Phase 3 (Months 7-12) |
| **Lock/Vest** | Smart contract controlled |

**Staking Benefits (Planned):**
- Earn passive 8BIT rewards
- Access exclusive tournaments
- Boosted daily rewards
- Governance voting power
- NFT badge eligibility

**Conservative Emissions:**
- Much slower than rewards pool
- Rewards long-term believers
- Adds utility beyond gaming
- Reduces sell pressure

---

### 💧 DEX Liquidity - 60,000,000 (12%)

**Purpose:** Deep trading liquidity on Uniswap V3

| Detail | Information |
|--------|-------------|
| **Amount** | 60,000,000 8BIT |
| **Value at Presale** | $30,000 USD |
| **Paired With** | $30,000+ USDC from presale |
| **Platform** | Uniswap V3 (Arbitrum) |
| **Lock Period** | 3-4 years |

**Why 12% is Strong:**
- Most projects: 5-10% liquidity
- **8-Bit Arcade: 12%** - deep, professional pool
- Minimizes slippage on large trades
- Professional, sustainable launch
- Prevents price manipulation
- Builds investor confidence

**Liquidity Pairs:**
- **Primary:** 8BIT/USDC (tournament fees, buybacks)
- **Future:** 8BIT/ETH (if successful, broader access)

**Lock Details:**
- Liquidity LP tokens locked via contract
- Cannot be removed by team
- Trustless, verifiable on-chain
- Lock period announced before launch

---

### 📢 Marketing & Partnerships - 15,000,000 (3%)

**Purpose:** Growth, listings, influencers, partnerships

| Detail | Information |
|--------|-------------|
| **Amount** | 15,000,000 8BIT |
| **Value at Presale** | $7,500 USD |
| **Usage** | Performance-based |
| **Accountability** | Public wallet tracking |

**Marketing Budget Uses:**
- Exchange listings (CEX/DEX)
- Influencer partnerships
- Gaming community outreach
- Tournaments with streamers
- Social media campaigns
- Partnership integrations

**Why Only 3%:**
- Small allocation = investor friendly
- $15K cash from presale for marketing too
- Performance-based spending only
- No dumping on holders
- Tokens used strategically, not sold

---

### 🏆 Tournament Prize Pools - 20,000,000 (4%)

**Purpose:** Initial tournament prize pool funding

| Detail | Information |
|--------|-------------|
| **Amount** | 20,000,000 8BIT |
| **Value at Presale** | $10,000 USD |
| **Usage** | Tournament prizes |
| **Lock/Vest** | Available immediately for tournaments |

**Purpose:**
- Funds initial tournament prize pools
- Ensures tournaments can run immediately at launch
- Provides runway while GameRewards emissions ramp up
- Attracts competitive players from day one

**Tournament Structure:**
- **Standard Weekly:** $1 entry, $25 prize pool
- **Standard Monthly:** $5 entry, $50 prize pool
- **High Roller Weekly:** $5 entry, $75 prize pool
- **High Roller Monthly:** $25 entry, $250 prize pool

**Note:** After these initial prizes are distributed, ongoing tournament prizes come from the GameRewards 150M emissions pool over 5 years.

---

### 👥 Team Allocation - 5,000,000 (1%)

**Purpose:** Team incentives and long-term alignment

| Detail | Information |
|--------|-------------|
| **Amount** | 5,000,000 8BIT |
| **Value at Presale** | $2,500 USD |
| **Vesting Period** | 2-3 years |
| **Lock/Vest** | Fully vested |

**Why Only 1%:**
- **Ultra-low team allocation**
- Most projects: 10-20% team allocation
- Shows maximum commitment to community
- Team earns through project success
- Zero ability to dump on investors

**Vesting Purpose:**
- Aligns team with long-term success
- Prevents immediate selling
- Builds investor trust
- Industry best practice

## Circulating Supply Timeline

### Launch (Day 1)

| Allocation | Amount | Status |
|------------|--------|--------|
| Token Sale | 200M | ✅ Circulating |
| Tournament Prizes | 20M | ✅ Available for prizes |
| DEX Liquidity | 60M | 🔒 Locked in pool |
| Marketing | 15M | ⚠️ Treasury (used strategically) |
| Team | 5M | 🔒 Vested |
| **Total Circulating** | **~200M** | **40% of supply** |

### Month 3

| Allocation | Amount | Status |
|------------|--------|--------|
| Initial Circulation | 200M | Circulating |
| Rewards Distributed | 7.5M | ✅ Earned by players |
| **Total Circulating** | **~207.5M** | **41.5%** |

### Year 1

| Allocation | Amount | Status |
|------------|--------|--------|
| Initial Circulation | 200M | Circulating |
| Rewards Distributed | 30M | ✅ Earned by players |
| **Total Circulating** | **~230M** | **46%** |

### Year 5 (All Rewards Distributed)

| Allocation | Amount | Status |
|------------|--------|--------|
| Initial Circulation | 200M | Circulating |
| Rewards Distributed | 150M | ✅ Earned by players |
| Staking Distributed | ~20M | Estimated |
| **Total Circulating** | **~370M** | **74%** |

**Note:** Actual circulating supply will be lower due to:
- Buyback and burn (deflationary)
- Locked staking tokens
- Lost wallet addresses

## Comparison to Other Projects

| Project | Token Sale | Rewards | Liquidity | Tournaments | Team | Marketing |
|---------|-----------|---------|-----------|-------------|------|-----------|
| **8-Bit Arcade** | **40%** | **40%** | **12%** | **4%** | **1%** | **3%** |
| Typical GameFi | 30% | 30% | 10% | 0% | 15% | 15% |
| Axie Infinity | 29% | 29% | 8% | 0% | 21% | 13% |
| DeFi Kingdoms | 25% | 40% | 5% | 0% | 20% | 10% |

**8-Bit Arcade Advantages:**
- ✅ Strong liquidity allocation (12% vs typical 5-10%)
- ✅ Ultra-low team allocation (1% vs typical 15-20%)
- ✅ Lowest marketing allocation (3% vs typical 10-15%)
- ✅ Dedicated tournament prize pool (4% upfront funding)
- ✅ Front-loaded rewards (5 years vs 20-30 years)
- ✅ More tokens to players and investors

## Transparency & Accountability

All allocations are:
- ✅ Publicly viewable on blockchain
- ✅ Tracked via Arbiscan explorer
- ✅ Smart contract enforced (not manual)
- ✅ Locked/vested where applicable
- ✅ Community can verify anytime

**Official Wallets:**
- Token Contract: [View on Arbiscan](https://arbiscan.io)
- Liquidity Pool: [View on Uniswap](https://info.uniswap.org)
- Rewards Contract: [View on Arbiscan](https://arbiscan.io)

## What's Next?

- [Token Sale Details](token-sale.md) - How to participate
- [Emissions Schedule](emissions.md) - When rewards are distributed
- [Market Cap Scenarios](market-scenarios.md) - What your tokens could be worth
- [Deflationary Model](deflationary.md) - How supply decreases

---

*Token distribution is enforced by smart contracts and publicly verifiable on-chain. See [Contract Documentation](../contracts/overview.md) for technical details.*
